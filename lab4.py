# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 18:06:44 2023
...
def Fuerza(r, x, y, z):
    dV = 4*epsilon*((m*sigma**m)/r**(m+2) - (n*sigma**n)/r**(n+2)) 
...
#Calculo de la energía para condiciones de contorno libres
    ...
    F = np.zeros((n_atom, 3)) #Define un array para guardar la componentes de la fuerza sobre cada átomo
    F_atomos = [] #Define una lista para guardar el módulo de la fuerza sobre cada átomo
    for i in range(n_atom):
        ...
        for j in range(n_atom):
            ...
            if radio == 0:
                ...
                F[i] += Fuerza(radio, dx, dy, dz) #Calcula las coordenadas de la fuerza sobre el átomo i
            else:
                None 
        E_atomos.append(Ei) #Añade la energía del átomo i a la lista de energías de cada átomo
        F_modulo = np.sqrt(F[i, 0]**2+F[i, 1]**2+F[i, 2]**2) #Calcula el módulo de la fuerza sobre el átomo i
        F_atomos.append(F_modulo) #Añade el módulo de la fuerza sobre el átomo i a la lista de fuerza sobre cada átomo
...
#Gráfico del módulo de la fuerza sobre cada átomo
fig3 = plt.figure()
ax3 = fig3.add_subplot(111, projection='3d')
mapacolor = ax3.scatter(x, y, z, c = F_atomos, cmap = 'hot')
ax3.set_title('Fuerza sobre cada átomo')
fig3.colorbar(mapacolor, label = 'Fuerza (eV/$\AA$)')
ax3.set_xlabel('x ($\AA$)')
ax3.set_ylabel('y ($\AA$)')
#ax3.set_zlabel('z ($\AA$)')
plt.show()

#Gráfico del vector fuerza sobre cada átomo
F_x = F[:,0]
F_y = F[:,1]
F_z = F[:,2]
fig4 = plt.figure()
ax4 = fig4.add_subplot(111,projection='3d')
ax4.quiver(x, y, z, 5*F[:,0], 5*F[:,1], 5*F[:,2],color='blue')
ax4.set_xlabel('x ($\AA$)')
ax4.set_ylabel('y ($\AA$)')
ax4.set_zlabel('z ($\AA$)')
#ax4.set_xlim(0,2.5)
